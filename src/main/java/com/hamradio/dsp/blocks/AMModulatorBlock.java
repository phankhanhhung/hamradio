...
public class AMModulatorBlock extends ProcessingBlock {

    private final NativeDSP dsp = new NativeDSP();
    private final float carrierFreq;
    private final int sampleRate;

    public AMModulatorBlock(String id, float carrierFreq, int sampleRate) {
        super(id);
    ...
    public void process(float[] input, float[] output, int numSamples) {
        float[] result = dsp.modulateAM(input, carrierFreq, sampleRate);
        if (result != null) {
